...
mod tests {
    ...
    // Count exact line matches in the printed flat output.
    fn count_matching_lines(output: &str, expected: &str) -> usize {
        output.lines().filter(|line| *line == expected).count()
    }

    #[test]
    // Verify basic YAML parsing and scalar conversion behavior.
    fn test_parse_config() {
        let config =
            parse_config(include_str!("../tests/parse.yaml")).expect("config should parse");
    ...
    #[test]
    // Verify an explicit empty YAML document parses as an empty mapping config.
    ...
    #[test]
    // Verify multiple YAML documents return the expected parse error.
    ...
    #[test]
    // Verify loading an empty config file returns an empty config tree.
    fn test_load_config_empty_file() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/empty.yaml");

        let config = load_config(&path).expect("empty config file should load");
    ...
    #[test]
    // Verify config files parse without the %YAML directive when --- is present.
    fn test_load_config_without_yaml_directive() {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/no-yaml-directive.yaml");

        let config = load_config(&path).expect("config should load without %YAML directive");

        assert_eq!(config["stats"]["enabled"].as_str(), Some("yes"));
    }

    #[test]
    // Verify config files parse without %YAML and without an explicit --- marker.
    fn test_load_config_without_yaml_directive_or_doc_start() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/no-yaml-directive-no-doc-start.yaml");

        let config = load_config(&path).expect("config should load without %YAML or ---");

        assert_eq!(config["stats"]["enabled"].as_str(), Some("yes"));
    }

    #[test]
    // Verify root includes and tagged includes are resolved correctly.
    fn test_load_config_includes() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/include.yaml");

        let config = load_config(&path).expect("config should load with includes");
    ...
    #[test]
    // Document current behavior: duplicate mapping keys (including `include`) are not supported.
    ...
    #[test]
    // Verify nested include paths are resolved relative to each include file.
    ...
    #[test]
    // Verify dotted keys from includes are applied as nested overrides.
    fn test_load_config_dotted_overrides() {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/flat-includes.yaml");

        let config = load_config(&path).expect("config should load with dotted overrides");
    ...
    #[test]
    // Verify flat printing for a config that uses include directives.
...
vars.address-groups.EXTERNAL_NET = any\n";
    ...
    #[test]
    // Verify flat printing after dotted-key overrides are applied.
...
vars.port-groups.DEV_SERVER_PORTS = [3000,4200]\n";
    ...
    #[test]
    // Mirror verify checks for included array structures in flat output.
