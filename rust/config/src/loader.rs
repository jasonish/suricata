...
/// Read and parse a Suricata YAML configuration document from a file.
...
// Read a configuration file from disk and parse it as YAML.
fn load_config_file(path: &Path) -> Result<Config, LoadError> {
    let input = std::fs::read_to_string(path)?;
    crate::parse_config(&input).map_err(LoadError::from)
}
...
fn load_include(include_dir: &Path, include_name: &str) -> Result<(Config, PathBuf), LoadError> {
    ...
    let included_dir = include_path
        ...
        .to_path_buf();
    let included = load_config_file(&include_path)?;
