    ...
    while (gv != NULL) {
        if (gv->type == DETECT_FLOWBITS) {
            ...
            if (fv->datatype == FLOWVAR_TYPE_STR) {
                const char *fvname = VarNameStoreLookupById(fv->idx, VAR_TYPE_FLOW_VAR);
                MemBufferWriteString(aft->buffer, "FLOWVAR:           \"%s\" => \"", fvname);
                for (i = 0; i < fv->data.fv_str.value_len; i++) {
                    if (isprint(fv->data.fv_str.value[i])) {
                        MemBufferWriteString(aft->buffer, "%c", fv->data.fv_str.value[i]);
                    } else {
                        MemBufferWriteString(aft->buffer, "\\%02X", fv->data.fv_str.value[i]);
                    }
                }
                MemBufferWriteString(aft->buffer, "\"\n");
            } else if (fv->datatype == FLOWVAR_TYPE_INT) {
                const char *fvname = VarNameStoreLookupById(fv->idx, VAR_TYPE_FLOW_INT);
                MemBufferWriteString(aft->buffer,
                        "FLOWINT:           \"%s\" =>"
                        " %" PRIu32 "\n",
        ...
        const Packet *p, TcpSegment *seg, void *data, const uint8_t *buf, uint32_t buflen)
{
    ...
    PrintRawDataToBuffer(aft->buffer->buffer, &aft->buffer->offset, aft->buffer->size, buf, buflen);
...
static TmEcode AlertDebugLogger(ThreadVars *tv, const Packet *p, void *thread_data)
{
    ...
    for (i = 0; i < p->alerts.cnt; i++) {
        ...
        MemBufferWriteString(aft->buffer,
                ...
                i, pa->s->msg, i, pa->s->gid, i, pa->s->id, i, pa->s->rev, i,
                pa->s->class_msg ? pa->s->class_msg : "<none>", i, pa->s->prio, i,
                pa->flags & PACKET_ALERT_FLAG_STREAM_MATCH
                        ? "STREAM"
                        : (pa->flags & PACKET_ALERT_FLAG_STATE_MATCH ? "STATE" : "PACKET"));
        if (pa->flags & PACKET_ALERT_FLAG_TX) {
            MemBufferWriteString(aft->buffer, "ALERT IN TX [%02d]:    %" PRIu64 "\n", i, pa->tx_id);
        } else {
            MemBufferWriteString(aft->buffer, "ALERT IN TX [%02d]:    N/A\n", i);
        }
        if (p->payload_len > 0) {
            MemBufferWriteString(aft->buffer,
                    "PAYLOAD LEN:         %" PRIu32 "\n"
        ...
        if ((pa->flags & PACKET_ALERT_FLAG_STATE_MATCH) ||
            ...
            int ret;
            uint8_t flag;
            if (!(PKT_IS_TCP(p)) || p->flow == NULL || p->flow->protoctx == NULL) {
                return TM_ECODE_OK;
            }
            ...
            ret = StreamSegmentForEach(
                    (const Packet *)p, flag, AlertDebugPrintStreamSegmentCallback, (void *)aft);
            if (ret < 0) {
    ...
    aft->file_ctx->Write((const char *)MEMBUFFER_BUFFER(aft->buffer), MEMBUFFER_OFFSET(aft->buffer),
            aft->file_ctx);
...
static TmEcode AlertDebugLogDecoderEvent(ThreadVars *tv, const Packet *p, void *thread_data)
{
    ...
    for (i = 0; i < p->alerts.cnt; i++) {
        ...
        MemBufferWriteString(aft->buffer,
                ...
                "ALERT CLASS [%02d]:  %s\n"
                "ALERT PRIO [%02d]:   %" PRIu32 "\n",
                i, pa->s->msg, i, pa->s->gid, i, pa->s->id, i, pa->s->rev, i, pa->s->class_msg, i,
                pa->s->prio);
    }
