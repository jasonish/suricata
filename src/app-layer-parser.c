    ...
    for (int flow_proto = 0; flow_proto < FLOW_PROTO_DEFAULT; flow_proto++) {
        for (AppProto alproto = 0; alproto < ALPROTO_MAX; alproto++) {
            if (!(alp_ctx.ctxs[flow_proto][alproto].internal_flags &
                ...
                alp_ctx.ctxs[flow_proto][alproto].stream_depth = stream_config.reassembly_depth;
            }
        }
    }
}
    ...
    for (uint8_t flow_proto = 0; flow_proto < FLOW_PROTO_DEFAULT; flow_proto++) {
        for (AppProto alproto = 0; alproto < ALPROTO_MAX; alproto++) {
            ...
            AppLayerParserDestroyProtocolParserLocalStorage(
                    ipproto, alproto, tctx->alproto_local_storage[flow_proto][alproto]);
        }
    }
        ...
        void *(*StateAlloc)(void *, AppProto), void (*StateFree)(void *))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateAlloc = StateAlloc;
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateFree = StateFree;
        ...
        void *(*LocalStorageAlloc)(void), void (*LocalStorageFree)(void *))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageFree = LocalStorageFree;
        ...
        int (*StateGetProgress)(void *alstate, uint8_t direction))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetProgress = StateGetProgress;
        ...
        uint8_t ipproto, AppProto alproto, void (*StateTransactionFree)(void *, uint64_t))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateTransactionFree = StateTransactionFree;
        ...
        uint8_t ipproto, AppProto alproto, uint64_t (*StateGetTxCnt)(void *alstate))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetTxCnt = StateGetTxCnt;
        ...
        uint8_t ipproto, AppProto alproto, void *(StateGetTx)(void *alstate, uint64_t tx_id))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetTx = StateGetTx;
                ...
                int event_id, const char **event_name, AppLayerEventType *event_type))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetEventInfoById =
                ...
                const char *event_name, int *event_id, AppLayerEventType *event_type))
{
    ...
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].StateGetEventInfo = StateGetEventInfo;
...
void *AppLayerParserGetProtocolParserLocalStorage(uint8_t ipproto, AppProto alproto)
{
    ...
    if (alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageAlloc != NULL) {
        r = alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageAlloc();
    }
        ...
        uint8_t ipproto, AppProto alproto, void *local_data)
{
    ...
    if (alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageFree != NULL) {
        alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].LocalStorageFree(local_data);
    }
 ...
 */
static AppLayerGetTxIterTuple AppLayerDefaultGetTxIterator(const uint8_t ipproto,
...
{
    uint64_t detect_flags = (dir & STREAM_TOSERVER) ? txd->detect_flags_ts : txd->detect_flags_tc;
    return detect_flags;
}

static inline void SetTxDetectFlags(
        AppLayerTxData *txd, const uint8_t dir, const uint64_t detect_flags)
{
        ...
        void *alstate, const uint8_t flags, bool tag_txs_as_inspected)
{
    ...
    const int state_done_progress =
            AppLayerParserGetStateProgressCompletionStatus(f->alproto, flags);
    ...
    if (tag_txs_as_inspected) {
        ...
        while (1) {
            AppLayerGetTxIterTuple ires =
                    IterFunc(ipproto, alproto, alstate, idx, total_txs, &state);
            if (ires.tx_ptr == NULL)
                break;
            ...
            if (likely(txd)) {
                ...
                if ((detect_flags & APP_LAYER_TX_INSPECTED_FLAG) == 0) {
                    ...
                    SCLogDebug("%p/%" PRIu64
                            ...
                            tx, idx, flags & STREAM_TOSERVER ? "toserver" : "toclient",
                            detect_flags);

                    SCLogDebug("%p/%" PRIu64 " out of order tx. Update inspect_id? %" PRIu64, tx,
                            idx, pstate->inspect_id[direction]);
    ...
    while (1) {
        ...
        if (txd && logger_expectation != 0) {
            ...
            if (tx_logged != logger_expectation) {
                SCLogDebug("%p/%" PRIu64 " skipping: logging not done: want:%" PRIx32
                           ", have:%" PRIx32,
        ...
        SCLogDebug("skipped? %s i %" PRIu64 ", new_min %" PRIu64, skipped ? "true" : "false", i,
                new_min);
        if (!skipped)
            new_min = i + 1;
        ...
        int *event_id, AppLayerEventType *event_type)
{
    ...
    int r = (alp_ctx.ctxs[ipproto_map][alproto].StateGetEventInfo == NULL)
                    ? -1
                    : alp_ctx.ctxs[ipproto_map][alproto].StateGetEventInfo(
        ...
        const char **event_name, AppLayerEventType *event_type)
{
    ...
    *event_name = (const char *)NULL;
    int r = (alp_ctx.ctxs[ipproto_map][alproto].StateGetEventInfoById == NULL)
                    ? -1
                    : alp_ctx.ctxs[ipproto_map][alproto].StateGetEventInfoById(
...
int AppLayerParserParse(ThreadVars *tv, AppLayerParserThreadCtx *alp_tctx, Flow *f,
        AppProto alproto, uint8_t flags, const uint8_t *input, uint32_t input_len)
{
    SCEnter();
    ...
    if (alstate == NULL || FlowChangeProto(f)) {
        ...
        SCLogDebug("alloced new app layer state %p (name %s)", alstate,
                AppLayerGetProtoName(f->alproto));
        ...
        SCLogDebug("using existing app layer state %p (name %s))", alstate,
                AppLayerGetProtoName(f->alproto));
    }
    ...
    if (!(f->flags & FLOW_NOPAYLOAD_INSPECTION) &&
            pstate->flags & APP_LAYER_PARSER_NO_INSPECTION_PAYLOAD) {
        FlowSetNoPayloadInspectionFlag(f);
        /* Set the no reassembly flag for both the stream in this TcpSession */
        if (f->proto == IPPROTO_TCP) {
            /* Used only if it's TCP */
...
static void ValidateParserProtoDump(AppProto alproto, uint8_t ipproto)
{
    ...
    printf("- StateGetTx %p StateGetTxCnt %p StateTransactionFree %p\n", ctx->StateGetTx,
            ctx->StateGetTxCnt, ctx->StateTransactionFree);
    printf("- GetTxData %p\n", ctx->GetTxData);
...
void AppLayerParserRegisterProtocolParsers(void)
{
    ...
    if (AppLayerProtoDetectConfProtoDetectionEnabled("tcp", "imap")) {
        if (AppLayerProtoDetectPMRegisterPatternCS(
                    IPPROTO_TCP, ALPROTO_IMAP, "1|20|capability", 12, 0, STREAM_TOSERVER) < 0) {
            SCLogInfo("imap proto registration failure");
            exit(EXIT_FAILURE);
        }
    } else {
        SCLogInfo("Protocol detection and parser disabled for %s protocol.", "imap");
    }
...
void AppLayerParserStatePrintDetails(AppLayerParserState *pstate)
{
    ...
    SCLogDebug("AppLayerParser parser state information for parser state p(%p). "
               ...
               "p->log_id(%" PRIu64 "), "
               "p->decoder_events(%p).",
            pstate, p->inspect_id[0], p->inspect_id[1], p->log_id, p->decoder_events);
...
{
    SCEnter();
    alp_ctx.ctxs[FlowGetProtoMapping(ipproto)][alproto].RegisterUnittests = RegisterUnittests;
    SCReturn;
}
...
static int AppLayerParserTest01(void)
{
    ...
    AppLayerParserRegisterStateFuncs(
            IPPROTO_TCP, ALPROTO_TEST, TestProtocolStateAlloc, TestProtocolStateFree);
    ...
    int r = AppLayerParserParse(
            NULL, alp_tctx, f, ALPROTO_TEST, STREAM_TOSERVER | STREAM_EOF, testbuf, testlen);
    FAIL_IF(r != -1);
...
static int AppLayerParserTest02(void)
{
    ...
    /* Register the Test protocol state and parser functions */
    AppLayerParserRegisterParser(IPPROTO_UDP, ALPROTO_TEST, STREAM_TOSERVER, TestProtocolParser);
    AppLayerParserRegisterStateFuncs(
            IPPROTO_UDP, ALPROTO_TEST, TestProtocolStateAlloc, TestProtocolStateFree);
