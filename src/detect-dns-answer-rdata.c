 ...
 * \file
 *
 * Detect keyword for DNS answer name: dns.answer.rdata.<bytes>
 */
...
#include "detect-dns-answer-rdata.h"
...
void DetectDnsAnswerRdataRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_DNS_ANSWER_RDATA].prefix = "dns.answer.rdata";
    sigmatch_table[DETECT_AL_DNS_ANSWER_RDATA].keywords = keywords;
    sigmatch_table[DETECT_AL_DNS_ANSWER_RDATA].desc = "DNS answer rdata sticky buffer";
    sigmatch_table[DETECT_AL_DNS_ANSWER_RDATA].url = "/rules/dns-keywords.html#dns-answer-name";
    sigmatch_table[DETECT_AL_DNS_ANSWER_RDATA].Setup = DetectSetup;
    sigmatch_table[DETECT_AL_DNS_ANSWER_RDATA].flags |= SIGMATCH_NOOPT;
    sigmatch_table[DETECT_AL_DNS_ANSWER_RDATA].flags |= SIGMATCH_INFO_STICKY_BUFFER;

    for (size_t i = 0; i < sizeof(keywords) / sizeof(keywords[0]); i++) {
	char keyword[PATH_MAX];

	snprintf(keyword, sizeof(keyword), "dns.answer.rdata.%s", keywords[i]);
	SCLogNotice("keyword = %s", keyword);
	...
	SCLogNotice("keyword registered: %s", keyword);
    }
}
