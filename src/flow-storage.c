...
void *SCFlowGetStorageById(const Flow *f, FlowStorageId id)
{
    return StorageGetById(f->storage, STORAGE_FLOW, id.id);
}

int SCFlowSetStorageById(Flow *f, FlowStorageId id, void *ptr)
{
    return StorageSetById(f->storage, STORAGE_FLOW, id.id, ptr);
}

void SCFlowFreeStorageById(Flow *f, FlowStorageId id)
{
...
static int FlowStorageTest01(void)
{
    ...
    FlowStorageId id1 = SCFlowStorageRegister("test", StorageTestFree);
    FAIL_IF(id1.id < 0);
    FlowStorageId id2 = SCFlowStorageRegister("variable", StorageTestFree);
    FAIL_IF(id2.id < 0);
    FlowStorageId id3 = SCFlowStorageRegister("store", StorageTestFree);
    FAIL_IF(id3.id < 0);
...
static int FlowStorageTest03(void)
{
    ...
    FlowStorageId id1 = SCFlowStorageRegister("test1", StorageTestFree);
    FAIL_IF(id1.id < 0);
    FlowStorageId id2 = SCFlowStorageRegister("test2", StorageTestFree);
    FAIL_IF(id2.id < 0);
    FlowStorageId id3 = SCFlowStorageRegister("test3", StorageTestFree);
    FAIL_IF(id3.id < 0);
