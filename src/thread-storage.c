...
void *SCThreadGetStorageById(const ThreadVars *tv, ThreadStorageId id)
{
    return StorageGetById(tv->storage, storage_type, id.id);
}

int SCThreadSetStorageById(ThreadVars *tv, ThreadStorageId id, void *ptr)
{
    return StorageSetById(tv->storage, storage_type, id.id, ptr);
}

void SCThreadFreeStorageById(ThreadVars *tv, ThreadStorageId id)
{
...
ThreadStorageId SCThreadStorageRegister(const char *name, void (*Free)(void *))
{
...
static int ThreadStorageTest01(void)
{
    ...
    ThreadStorageId id1 = SCThreadStorageRegister("test", StorageTestFree);
    FAIL_IF(id1.id < 0);

    ThreadStorageId id2 = SCThreadStorageRegister("variable", StorageTestFree);
    FAIL_IF(id2.id < 0);

    ThreadStorageId id3 = SCThreadStorageRegister("store", StorageTestFree);
    FAIL_IF(id3.id < 0);
...
static int ThreadStorageTest03(void)
{
    ...
    ThreadStorageId id1 = SCThreadStorageRegister("test1", StorageTestFree);
    FAIL_IF(id1.id < 0);

    ThreadStorageId id2 = SCThreadStorageRegister("test2", StorageTestFree);
    FAIL_IF(id2.id < 0);

    ThreadStorageId id3 = SCThreadStorageRegister("test3", StorageTestFree);
    FAIL_IF(id3.id < 0);
